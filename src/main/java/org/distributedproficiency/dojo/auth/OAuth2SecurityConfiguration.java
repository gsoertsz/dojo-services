...
import org.distributedproficiency.dojo.DojoUserManagementConfiguration;
 ...
 *  
 */
@Import({DojoUserManagementConfiguration.class})
@Configuration
public class OAuth2SecurityConfiguration {
	...
	protected static class OAuth2Config extends
		...
		@Autowired
		private DojoUserManagementService dojoUserManagementService;
		...
		public OAuth2Config() throws Exception {
			...
			.scopes("read","write").resourceIds("registration", "appointments", "treatments", "registrations", "patients", "queue")
			.and()
			// Create a second client that only has "read" access to the
			// video service
			.withClient("mobileReader").authorizedGrantTypes("password")
			.authorities("ROLE_CLIENT")
			.scopes("read").resourceIds("appointments", "treatments", "patients", "queue")
			.accessTokenValiditySeconds(3600).and().build();

			// Create a series of hard-coded users. 

			// ADMIN / PATIENT authorities are Roles as in hasRole(...)			
			//UserDetailsService svc = impatientUserManagementService;

			//					Arrays.asList(
			//							User.create("admin", "pass", ImpatientUserRole.ADMIN),
			//							User.create("gsoertsz", "pass", ImpatientUserRole.PATIENT),
			//							User.create("alana", "pass", ImpatientUserRole.PATIENT)
			//							,
		...
		public ClientDetailsService clientDetailsService() throws Exception {
			if (combinedService_ == null) combinedService_ = new ClientAndUserDetailsService(csvc, dojoUserManagementService);
